...
from static.constant import Fold, DrugAE_SaveBase, CellAE_SaveBase, MTLSA_SaveBase, MTLSA_Result
...
demensions = [(32, 256), (64, 256), (128, 256), (512, 256), (512, 512), (128, 128), (128, 512), (128, 1024),
              (128, 2048)]
...
def test(model, drugEncoder, cellLineEncoder, test_dataloader, test_num, mse, cce, test_fold):
    ...
    with torch.no_grad():
        ...
        y_true1 = y_true1[0:n]
        y_true3 = y_true3[0:n]
        y_pred1 = (y_pred1[0:n] + y_pred1[n:]) / 2  # A-B和B-A取平均
        ...
        print("test result:\n" + "Synergy:\n" + str(synergy_result) + "\nSensitivity:\n" + str(
            sensitivity_result) + "\n")
        with open(MTLSA_Result, 'a') as file:
            file.write("---------- Test ----------\n")
...
for drug_output, cell_output in demensions:
    ...
    save_path_base = MTLSA_SaveBase + str(drug_output) + "-" + str(cell_output) + "/"
    ...
    for fold_test in range(0, Fold):
        ...
        save_path = save_path_base + "fold_" + str(fold_test) + ".pth"
